...
- `components/` — переиспользуемые Compose-компоненты: `LineChart.kt` (самодельный график),
  `HeartRateWidget.kt` (карточка пульсометра: подключить/отключить, текущий пульс, счётчик
  записанных показаний, диалог выбора датчика, кнопки «Забыть датчик»/«Найти другой»).
